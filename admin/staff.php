...
<body class="bg-gray-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-4xl bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <h2 class="text-2xl font-bold bg-green-500 text-white text-center py-4">Staff List</h2>  

        <form action="./staffadd.php" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4 px-6 py-4 bg-gray-50 border-b border-gray-300">
            <input type="hidden" name="csrf_token" value="<?php echo GenerateCsrfToken(); ?>">
            <div>
                <label for="staffname" class="block text-sm font-medium text-gray-700 mb-1">Name :</label>
                <input type="text" name="staffname" placeholder="Enter Name For Staff" id="staffname" class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-green-400" >
            </div>
            <div>
                <label for="staffrole" class="block text-sm font-medium text-gray-700 mb-1">Role :</label>
                <select name="staffrole" id="staffrole" class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-green-400">
                    <option value="" selected>Select Role Of Staff</option>
            ...
            <div class="flex items-end">
                <button type="submit" class="w-full bg-green-500 hover:bg-green-600 text-white font-semibold py-2 rounded" name="addstaff">Add Staff</button>
            </div>
        </form>
            ...
            <table class="w-full table-auto border border-gray-300">
                ...
                <tbody class="text-gray-800 text-base">
                    ...
                    <tr class="odd:bg-green-50 even:bg-green-100 hover:bg-green-200 transition-colors">
                        <td class="px-6 py-4 border border-gray-300"><?php echo htmlspecialchars(strip_tags($row['name'])); ?></td>
                        <td class="px-6 py-4 border border-gray-300"><?php echo htmlspecialchars(strip_tags($row['role'])); ?></td>
                        <td class="px-6 py-4 border border-gray-300">
                            <div class="flex justify-evenly">
                                <form action="./staffdelete.php" method="POST" onsubmit="return confirmDelete();">
                                    <input type="hidden" name="csrf_token" value="<?php echo GenerateCsrfToken(); ?>">
